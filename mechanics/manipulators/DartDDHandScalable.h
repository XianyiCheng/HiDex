...
class DartDDHandScalable: public virtual DartManipulatorTemplate {
...
public:
    ...
    DartDDHandScalable(int fingertype, double scale);
    
    DartDDHandScalable(const DartDDHandScalable& dd): DartManipulatorTemplate(dd), 
        radius(dd.radius), fingertype(dd.fingertype),
        finger_length(dd.finger_length), hand_pose(dd.hand_pose), scale(dd.scale){}
