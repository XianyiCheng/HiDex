...
namespace HMP
{
  ...
  public:
    ...
    virtual double get_result(Node<State> *node)
    {
      ...
      if (node->m_visits > 0)
      {
        // if the terminal node has been evaluated before, we will just return its previous result
        // This is valid unless there is randomness in the result evaluation process
        return node->m_value;
      }
      ...
      Node<State2> *final_node_2 = tree2.search_tree(compute_options_2_1st,compute_options_2);
        ...
        while (!this->is_terminal(node))
        {
          ...
          else if (node->m_type == "mode")
          {
            if (node->number_of_next_actions <= pow(node->m_visits, m_alpha))
            {
              ...
              {
                // couldn't find a new path through search, update this node with
                // zero reward for one simulation
                node->update(0.0);
              }
              break;
            }
