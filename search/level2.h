...
public:
  ...
  virtual int select_action(Node<State> *node) {
    ...
    if ((node->m_parent != nullptr)) {
      action_idx = this->m_task->action_selection_policy_level2(node->m_state,
    ...
    if ((action_idx == -1) || (action_idx >= node->number_of_next_actions)) {
      ...
      for (int k = 0; k < node->number_of_next_actions; ++k) {
        ...
        if (std::find(explored_actions.begin(), explored_actions.end(), k) ==
            explored_actions.end()) {
          // k has not been explored
          double estimate_value =
              this->m_task->estimate_next_state_value(node->m_state, k);

          double U =
              estimate_value + this->ita * (1 / node->number_of_next_actions) *
                                   std::sqrt(double(node->m_visits)) / (1 + 0);

          if (U > U_max) {
            U_max = U;
            action_idx = k;
            action_idx = randi(node->number_of_next_actions);
          }
        }
      }
    }
  ...
  Node<State> *search_tree(const MCTSOptions &compute_option_1st_iter,
    ...
    while (!this->is_terminal(current_node)) {
      if (iter == 0) {
        this->grow_tree(
            current_node,
            compute_option_1st_iter); // grow the tree to the end (simulation)
