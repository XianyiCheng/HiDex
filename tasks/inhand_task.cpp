...
void InhandTASK::sample_likely_feasible_finger_idx(State2 state, double t_change, int number, std::vector<int> *finger_idxs)
{
  // sample the finger idxes that are likely to be feasible in this state and move to the next state
  ...
  for (int k = 0; k < this->number_of_robot_contacts; ++k)
  {
    ...
    if (check_next_step)
    {
      this->m_world->getRobot()->points_in_workspace(k, object_surface_world_current, &pws);
      this->m_world->getRobot()->points_in_workspace(k, object_surface_world_next, &pws_next);
      for (int pt_i : pws)
      {
        if (std::find(pws_next.begin(), pws_next.end(), pt_i) != pws_next.end())
        {
    ...
    {
      this->m_world->getRobot()->points_in_workspace(k, object_surface_world_next,
                                                     &points_in_workspaces[k]);
    }
  }
  ...
  for (int finger_stay = 0; finger_stay < this->number_of_robot_contacts; ++finger_stay)
  {
    for (int idx = 0; idx < combination(this->number_of_robot_contacts, finger_stay); ++idx)
    {
      std::vector<int> remain_idx = combination_set(this->number_of_robot_contacts, finger_stay, idx);
      bool is_remain_ws = true; // is all the remaining fingers in workspace, if not, skip
      ...
      for (int ir : remain_idx)
      {
        ...
        if (std::find(points_in_workspaces[ir].begin(), points_in_workspaces[ir].end(), finger_loc) == points_in_workspaces[ir].end())
        {
          is_remain_ws = false;
          break;
        }
  ...
  if (remain_idxes.size() == 0)
  {
    ...
    for (int iter = 0; iter < number; ++iter)
    {
      ...
      for (int k_finger = 0; k_finger < this->number_of_robot_contacts; ++k_finger)
      {
        // sample -1
        // sample relocation
        // sample stay
        ...
        if (pre_fingertips[k_finger] == -1) // if the fingertip was not touching
        {
          ...
          if (randd() < 0.8)
          {
            locs[k_finger] = points_in_workspaces[k_finger][randi(points_in_workspaces[k_finger].size())];
        ...
        else // if the fingertip was thouching
        {
          ...
          // out of ws
          if (std::find(points_in_workspaces[k_finger].begin(), points_in_workspaces[k_finger].end(), pre_fingertips[k_finger]) == points_in_workspaces[k_finger].end())
          {
            if (randd() < 0.7)
            {
              // change
      ...
      if (dynamic_feasibility && remain_dynamic_feasibility)
      {
        ...
        // VectorXd mnp_config = this->get_robot_config_from_action_idx(check_idx);
        // bool ik_next;
        // if (check_next_step)
        // {
        //   ik_next = this->m_world->getRobot()->ifIKsolution(mnp_config, x_object_next);
        // }
        // bool ik = this->m_world->getRobot()->ifIKsolution(mnp_config, x_object);
        // bool if_feasible = this->is_finger_valid(check_idx, t_change);
  ...
  for (int iter = 0; iter < number; ++iter)
  {
    ...
    for (int k_finger = 0; k_finger < this->number_of_robot_contacts; ++k_finger)
    {
      if (std::find(remain_idxes[idx].begin(), remain_idxes[idx].end(), k_finger) != remain_idxes[idx].end())
      {
        locs[k_finger] = pre_fingertips[k_finger];
      }
      else
