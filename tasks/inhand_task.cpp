...
void InhandTASK::initialize(
    ...
    long int start_finger_idx, long int goal_finger_idx, double goal_thr,
...
double InhandTASK::evaluate_path(const std::vector<State2> &path) {
  ...
  // compute finger change reward
  double total_finger_changes = this->total_finger_change_ratio(path);

  double x_finger = total_finger_changes / double(path.back().t_max);
  ...
  if (this->grasp_measure_charac_length <= 0.0) {
    ...
    //   if (grasp_d > max_grasp_d) {
    //     max_grasp_d = grasp_d;
    //   }
    // }
    // double x_grasp = max_grasp_d;
...
std::vector<int>
InhandTASK::get_finger_locations(long int finger_location_index) {
...
long int InhandTASK::pruning_check_w_transition(
    const Vector7d &x, const Vector7d &x_pre, const VectorXi &cs_mode,
    const Vector6d &v, const std::vector<ContactPoint> &envs,
...
bool InhandTASK::is_valid_transition(long int pre_finger_idx,
