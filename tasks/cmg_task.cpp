      ...
      std::vector<VectorXi> mode_to_extend;
      {
                     ...
                     shared_rrt->angle_weight) < d_zero)
        {
          bool is_pass_pruning =
              this->pruning_check(shared_rrt->nodes[near_idx].config, v,
...
bool CMGTASK::is_finger_valid(int finger_idx, int timestep)
{
  ...
  // update to the x in the next step if timestep < total_timesteps - 1, otherwise will check for zero velocity
  if (timestep < this->saved_object_trajectory.size() - 1)
  {
    x_object_next =
        this->saved_object_trajectory[timestep + 1].m_pose;
  }
  else
  {
  ...
  if (finger_idx != 0)
  {
    ...
    // if there is no ik solution, not valid
    if (!this->m_world->getRobot()->ifIKsolution(mnp_config,
                                                 x_object))
    {
      return false;
    }
  ...
  // check if there is quasistatic, or quasidynamic
  {
    ...
    // x_object,
    //                                                  &fingertips);
    this->m_world->getRobot()->Fingertips2PointContacts(fingertips,
                                                        &mnps);
  }
  ...
  {
    dynamic_feasibility = isQuasistatic(
        mnps, this->saved_object_trajectory[timestep].envs, v,
        this->f_gravity, x_object, this->mu_env, this->mu_mnp,
        this->cons.get());
  }
  else if (this->task_dynamics_type == CMG_QUASIDYNAMIC)
  {
    ...
    //     this->cons.get());
    dynamic_feasibility =
        true; // TODO: implement quasidynamic with velocity
  }

  return dynamic_feasibility;
}

bool CMGTASK::is_valid_transition(const CMGTASK::State2 &state, const CMGTASK::State2 &prev_state)
{
  ...
  if (if_relocate)
  {
    ...
    bool dynamic_feasibility = isQuasistatic(
        ...
        ss_mode_relocate, this->f_gravity, this->saved_object_trajectory[state.timestep].m_pose, this->mu_env,
        this->mu_mnp, this->cons.get());
    return dynamic_feasibility;
  }
  return true;
