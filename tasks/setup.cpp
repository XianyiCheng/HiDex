...
void load_surface_contacts(const std::string &file_name, std::vector<ContactPoint> *pts, double scale_x, double scale_y, double scale_z, bool negate_normal = false)
{
...
void load_task(std::shared_ptr<TASK> task, const YAML::Node &config)
{
    ...
    {
        std::vector<double> box_l = config["box_object"]["shape"].as<std::vector<double>>();
        SkeletonPtr object =
        ...
        bool negate_normal = config["box_object"]["negate_contact_normal"].as<bool>();
        load_surface_contacts(config["box_object"]["contact_file"].as<std::string>(), &surface_pts, box_l[0] / 2, box_l[1] / 2, box_l[2] / 2, negate_normal);
    }
    else if (config["mesh_object"])
    {
        ...
        SkeletonPtr object = createFreeObjectfromMesh(
            "mesh_object",
            config["mesh_object"]["mesh_file"].as<std::string>(),
            Vector3d(scale, scale, scale));
        world->addObject(object);

        std::cout << "Loading contacts" << std::endl;
        bool negate_normal = config["box_object"]["negate_contact_normal"].as<bool>();
        load_surface_contacts(config["mesh_object"]["contact_file"].as<std::string>(), &surface_pts, scale, scale, scale, negate_normal);
    }
    else
    {
    ...
    std::cout << "Number of surface contact points: " << surface_pts.size() << std::endl;
    ...
    double mu_env = config["environment_object_friction_coefficient"].as<double>();

    std::vector<double> object_w = config["object_weight"].as<std::vector<double>>();
    Vector6d f_g = Eigen::Map<Vector6d>(object_w.data());
    std::vector<double> object_I = config["object_inertia"].as<std::vector<double>>();
    ...
    rrt_options.goal_biased_prob = config["rrt_options"]["goal_biased_prob"].as<double>();

    rrt_options.x_ub = Eigen::Map<Vector3d>(config["rrt_options"]["sample"]["position_upper_bound"].as<std::vector<double>>().data());
    rrt_options.x_lb = Eigen::Map<Vector3d>(config["rrt_options"]["sample"]["position_lower_bound"].as<std::vector<double>>().data());

    rrt_options.eps_trans = config["rrt_options"]["extend_epsilon"]["translation"].as<double>();
    rrt_options.eps_angle = (3.14 / 180.0) * config["rrt_options"]["extend_epsilon"]["rotation_degree"].as<double>();
    rrt_options.sampleSO3 = config["rrt_options"]["sample"]["rotation"]["SO3_enabled"].as<bool>();
    rrt_options.sample_rotation_axis = Eigen::Map<Vector3d>(config["rrt_options"]["sample"]["rotation"]["axis"].as<std::vector<double>>().data());

    // Whether to refine the trajectory in level 2
    bool is_refine = config["refine_object_trajectory"]["enabled"].as<bool>();
    double refine_dist = config["refine_object_trajectory"]["distance"].as<double>();
    ...
    {
        n_robot_contacts = config["free_sphere_robot"]["number_of_contacts"].as<int>();
        ...
        if (config["free_sphere_robot"]["workspace_limits"])
        {
            ...
            for (int i = 1; i <= n_robot_contacts; ++i)
            {
                ...
                {
                    std::vector<double> lim = config["free_sphere_robot"]["workspace_limits"][box_name.c_str()].as<std::vector<double>>();
        ...
        rpt->is_patch_contact = config["free_sphere_robot"]["patch_contact"].as<bool>();
        world->addRobot(rpt);
    }
    else if (config["delta_array"])
    {
        ...
        for (int i = 1; i <= 20; ++i)
        {
            ...
            {
                std::vector<double> loc =
                    config["delta_array"]["locations"][robot_name.c_str()].as<std::vector<double>>();
        ...
        {
            std::cout << "Please specify robot locations for the delta array. Exit program." << std::endl;
            exit(0);
        }
    ...
    {
        std::cout << "Dexterous direct drive hand options is to be implemented. Exit Program." << std::endl;
        exit(0);
    }
    else
    {
    ...
    //
    task->set_task_parameters(goal_thr, wa, wt, charac_len, mu_env,
...
void load_start_and_goal_poses(std::shared_ptr<TASK> task, const YAML::Node &config)
{
    Vector7d x_start;
    Vector7d x_goal;
    std::vector<double> start_pose = config["object_start_pose"].as<std::vector<double>>();
    std::vector<double> goal_pose = config["object_goal_pose"].as<std::vector<double>>();
    x_start << start_pose[0], start_pose[1], start_pose[2], start_pose[3], start_pose[4], start_pose[5], start_pose[6];
    x_goal << goal_pose[0], goal_pose[1], goal_pose[2], goal_pose[3], goal_pose[4], goal_pose[5], goal_pose[6];
    task->set_start_and_goal(x_start, x_goal);
}

void load_reward_functions(std::shared_ptr<TASK> task, const YAML::Node &config)
{
    task->grasp_measure_charac_length = config["grasp_measure_charac_length"].as<double>();
    ...
    {
        std::cout << "No valid level 1 reward function specified. Exit program." << std::endl;
        exit(0);
    }
    ...
    {
        std::cout << "No valid level 2 reward function specified. Exit program." << std::endl;
        exit(0);
    }
...
void load_mcts_options(HMP::HierarchicalComputeOptions &compute_options, const YAML::Node &config)
{
    ...
    compute_options.l1.max_iterations = config["mcts_options"]["l1_max_iterations"].as<int>();
    compute_options.l2_1st.max_iterations =
        config["mcts_options"]["l2_1st_max_iterations"].as<int>();
    compute_options.l2.max_iterations = config["mcts_options"]["l2_max_iterations"].as<int>();
