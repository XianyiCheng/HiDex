...
namespace HMP
{
    ...
    class Level2TreeBaseline : public Tree<State, Task>
    {
    public:
        typedef typename Tree<State, Task>::Action Action;

        bool if_found_best_solution = false;

        Level2TreeBaseline() {}
        Level2TreeBaseline(std::shared_ptr<Task> task, State start_state)
        {
        ...
        virtual void grow_tree(Node<State> *grow_node, const MCTSOptions &options)
        {
                 ...
                 iter <= options.max_iterations || options.max_iterations < 0; ++iter)
            {
                ...
                if (this->if_check_time)
                {
                    this->check_time(options.max_time);
                    if (this->time_up)
                    {
                ...
                if ((this->m_task->total_finger_change_ratio(
                this->backtrack_state_path(node)) < 1.1) && this->is_terminal(node)){
                    std::cout << "reached optimal solution" << std::endl;
                    if_found_best_solution = true;
                    break;
                }
        ...
        virtual double get_result(Node<State> *node)
        {
            ...
            double x = total_finger_changes / double(this->m_task->saved_object_trajectory.size());
        ...
        virtual Action select_action(Node<State> *node)
        {
            ...
            // cretae a new child
            int t;
            if ((node->m_state.finger_index == -1) && node->m_state.timestep == 0)
            {
                t = 0;
            }
            else
            {
                t = node->m_state.timestep + 1;
            }
            bool if_valid = false;
                                 ...
                                 const MCTSOptions &compute_options)
        {
            ...
            if (!this->is_terminal(current_node))
            {
                ...
                if ((if_early_stop)||this->if_found_best_solution)
                {
            ...
            this->total_time = this->elasped_time();
            std::cout << "Total time: " << this->total_time << std::endl;
            return current_node;
        }
    };
