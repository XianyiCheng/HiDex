                               ...
                               const VectorXd &manipulator_config)
{
  ...
  for (counter = 0; counter < max_counter; counter++)
  {
    ...
    if (counter == 0){
      if (if_provide_mnp_config){
        ...
        if (selected_finger_idx == -1){
          ...
          selected_mnp_config = this->get_robot_config_from_action_idx(selected_finger_idx);
        }
      }
    }
    ...
    if (this->shared_rrt->dist(x, x_last_check) > check_distance)
    {
      ...
      bool pass_collision_n_ik = this->robot_collision_n_ik_check(x, selected_mnp_config);
...
bool TASK::robot_collision_n_ik_check(const Vector7d &x, const VectorXd &mnp_config)
{
                             ...
                             const std::vector<ContactPoint> &envs)
{
  ...
  //       v, mnps, envs, env_mode, this->f_gravity, this->object_inertia, x,
  //       this->mu_env, this->mu_mnp, this->wa, this->wt, 10.0, this->cons.get(),
  //       QUASIDYNAMIC_RANGE);
  ...
  //       bool test_quasistatic_feasibility = isQuasistatic(mnps, envs, env_mode, this->f_gravity, x, this->mu_env,
  //                       this->mu_mnp, this->cons.get());
  //       std::cout << "test quasistatic feasibility: " << test_quasistatic_feasibility
  //             << std::endl;
