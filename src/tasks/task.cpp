...
#define QUASIDYNAMIC_RANGE \
  0.3 // (-1,1) 1 exactly satisfy the accelerate direction, -1: whatever is ok
...
bool TASK::forward_integration(const Vector7d &x_start, const Vector7d &x_goal,
                               ...
                               const VectorXd &manipulator_config)
{
  ...
  VectorXd selected_mnp_config;
  Vector7d x_last_check;
  x_last_check << -100, -100, -100, -100, -100, -100, -100;
  ...
  for (counter = 0; counter < max_counter; counter++)
  {
    ...
    if (counter == 0)
    {
      ...
      {
        selected_mnp_config = manipulator_config;
      }
      else
      {
        ...
        {
          selected_mnp_config =
              this->get_robot_config_from_action_idx(selected_finger_idx);
        }
      }
    }

    if (this->shared_rrt->dist(x, x_last_check) > check_distance)
    {
      x_last_check = x;
    ...
    {
      bool pass_collision_n_ik =
          this->robot_collision_n_ik_check(x, selected_mnp_config);
      if (!pass_collision_n_ik)
      {
        break;
      }
    }
  ...
  for (int kk = 0; kk < this->search_options.max_samples; kk++)
  {
    ...
    for (const auto &cs_mode : extendable_cs_modes)
    {
      ...
      for (const auto &check_mode : check_modes)
      {
        ...
        {
          // If not save manipulator config during search, do a relaxed check:
          // does there exist any feasible manipulator config?
        ...
        {
          // If save and search the manipulator config, we need to use the saved
          // manipulator config to do the pruning check, and update the
          // manipuator config
          VectorXd new_manipulator_config;
          is_pass_pruning = this->pruning_check_w_manipulator_config(
              shared_rrt->nodes[near_idx].config, cs_mode, v,
              shared_rrt->nodes[near_idx].envs,
              shared_rrt->nodes[near_idx].manipulator_config,
  ...
  if (if_relocate)
  {
    ...
    else if (this->task_dynamics_type == "quasidynamic")
    {
      ...
      dynamic_feasibility = isQuasidynamic(
          ...
          this->saved_object_trajectory[state.timestep].m_pose, this->mu_env,
          this->mu_mnp, this->wa, this->wt, 1.0, this->cons.get(),
          QUASIDYNAMIC_RANGE);
      // return true;
    }
    else
                                        ...
                                        const std::vector<ContactPoint> &envs)
{
  ...
  else if (this->task_dynamics_type == "quasidynamic")
  {
    ...
    dynamic_feasibility = isQuasidynamic(
        v, mnps, envs, env_mode, this->f_gravity, this->object_inertia, x,
        this->mu_env, this->mu_mnp, this->wa, this->wt, h_time,
        this->cons.get(), QUASIDYNAMIC_RANGE);
  }
    ...
    const std::vector<ContactPoint> &envs)
{
  ...
  if (fingertips.size() != 0)
  {
    ...
    if (!this->robot_collision_n_ik_check(x, mnp_config))
    {
      return false;
    }
...
bool TASK::robot_collision_n_ik_check(const Vector7d &x,
                                      const VectorXd &mnp_config)
{
                             ...
                             const std::vector<ContactPoint> &envs)
{
  ...
  //   std::vector<ContactPoint> fingertips;
  //   fingertips.push_back(ContactPoint(Vector3d(-0.5, 0, 0), Vector3d(0, 0,
  //   1))); std::vector<ContactPoint> mnps;
  //   this->m_world->getRobot()->Fingertips2PointContacts(fingertips, &mnps);
  //   bool test_dynamic_feasibility = isQuasidynamic(
  //       v, mnps, envs, env_mode, this->f_gravity, this->object_inertia, x,
  //       this->mu_env, this->mu_mnp, this->wa, this->wt, 10.0,
  //       this->cons.get(), QUASIDYNAMIC_RANGE);
  //   std::cout << "test dynamic feasibility: " << test_dynamic_feasibility
  //             << std::endl;

  //     std::vector<ContactPoint> envs1;
  //   envs1.push_back(
  //       ContactPoint(Vector3d(0.5, 0.5, -0.5), Vector3d(0, 0,
  //       1), 9.99868e-05));
  //   envs1.push_back(
  //       ContactPoint(Vector3d(0.5, -0.5, -0.5), Vector3d(0, 0,
  //       1), 9.99868e-05));
  ...
  //         bool test_dynamic_feasibility1 = isQuasidynamic(
  //       v, mnps, envs1, env_mode, this->f_gravity, this->object_inertia, x,
  //       this->mu_env, this->mu_mnp, this->wa, this->wt, 10.0,
  //       this->cons.get(), QUASIDYNAMIC_RANGE);
  //       // const std::vector<ContactPoint> &mnps,
  //       //            const std::vector<ContactPoint> &envs,
  //       //            const VectorXi &env_mode, const Vector6d &f_ext_w,
  //       //            const Vector7d object_pose, double mu_env, double
  //       mu_mnp,
  //       //            ContactConstraints *cons
  //       bool test_quasistatic_feasibility = isQuasistatic(mnps, envs,
  //       env_mode, this->f_gravity, x, this->mu_env,
  //                       this->mu_mnp, this->cons.get());
  //       std::cout << "test quasistatic feasibility: " <<
  //       test_quasistatic_feasibility
  //             << std::endl;
  //   std::cout << "test dynamic feasibility1: " << test_dynamic_feasibility1
  //             << std::endl;
  ...
  //   std::cout << "f_gravity" << this->f_gravity.transpose() << std::endl;
  //   std::cout << "object_inertia" << this->object_inertia << std::endl;
  //   std::cout << "x: " << x.transpose() << std::endl;
  //   std::cout << "mu_env: " << this->mu_env << std::endl;
  //   std::cout << "mu_mnp: " << this->mu_mnp << std::endl;
  //   std::cout << "wa: " << this->wa << std::endl;
  //   std::cout << "wt: " << this->wt << std::endl;
  //   std::cout << "QUASIDYNAMIC_RANGE: " << QUASIDYNAMIC_RANGE << std::endl;
  // }
...
bool TASK::randomRelocateFingers(const VectorXd &mnp_config, Vector7d x,
                                 Vector6d v,
                                 const std::vector<ContactPoint> &envs,
                                 const VectorXi &env_mode, VectorXd &new_config,
                                 int samples)
{
  this->m_world->updateObjectPose(x);
  ...
  for (int k_sample = 0; k_sample < samples; k_sample++)
  {
    ...
    bool isremainbalance = false;
    {
      ...
      if (this->task_dynamics_type == "quasistatic")
      {
        ...
        isremainbalance = isQuasistatic(remain_grasp_mnps, envs, relocate_mode,
                                        this->f_gravity, x, this->mu_env,
                                        this->mu_mnp, this->cons.get());
      }
      else if (this->task_dynamics_type == "quasidynamic")
      {
        ...
        // Vector6d v_relocate;
        // v_relocate.setZero();
        isremainbalance = isQuasidynamic(
            v, remain_grasp_mnps, envs, relocate_mode, this->f_gravity,
            this->object_inertia, x, this->mu_env, this->mu_mnp, this->wa,
            this->wt, h_time, this->cons.get(), QUASIDYNAMIC_RANGE);
      }
    }
    if (!isremainbalance)
    {
    ...
    bool isbalance = false;
    {
      ...
      if (this->task_dynamics_type == "quasistatic")
      {
        ...
        isremainbalance =
            isQuasistatic(grasp_mnps, envs, env_mode, this->f_gravity, x,
                          this->mu_env, this->mu_mnp, this->cons.get());
      }
      else if (this->task_dynamics_type == "quasidynamic")
      {
        double h_time = 10.0;
        isremainbalance = isQuasidynamic(
            v, grasp_mnps, envs, env_mode, this->f_gravity,
            this->object_inertia, x, this->mu_env, this->mu_mnp, this->wa,
            this->wt, h_time, this->cons.get(), QUASIDYNAMIC_RANGE);
      }
    }
...
bool TASK::pruning_check_w_manipulator_config(
    const Vector7d &x, const VectorXi &cs_mode, const Vector6d &v,
    const std::vector<ContactPoint> &envs, const VectorXd &robot_config,
    VectorXd &new_robot_config, bool if_allow_transition)
{

  // Check if current robot configuration is valid
  if (robot_config.size() == 0)
  {
    int selected_finger_idx = this->pruning_check(x, cs_mode, v, envs);
    if (selected_finger_idx == -1)
    {
      return false;
    }
  ...
  std::vector<ContactPoint> mnps;
  std::vector<ContactPoint> fingertips;
  this->m_world->getRobot()->getFingertipsOnObject(robot_config, x,
                                                   &fingertips);
  this->m_world->getRobot()->Fingertips2PointContacts(fingertips, &mnps);
  ...
  // If current robot config is not valid, check if can find valid transition
  // and update new_robot_config

  VectorXd relocate_robot_config;
  int max_sample = 30;

  bool is_relocatable = this->randomRelocateFingers(
      robot_config, x, v, envs, env_mode, relocate_robot_config, max_sample);
...
bool TASK::is_finger_valid(long int finger_idx, int timestep)
{
  ...
  if (timestep < this->saved_object_trajectory.size() - 1)
  {
    ...
    {
      // double check_distance = 0.2;
      // for (auto x_v: this->saved_object_trajectory[timestep + 1].m_path) {
      //   if (this->shared_rrt->dist(x_v, x_object) >= check_distance) {
      //     v = v = compute_rbvel_body(x_object, x_v);
      //     break;
      //   }
      // }
  ...
  else if (this->task_dynamics_type == "quasidynamic")
  {
    ...
    dynamic_feasibility =
        isQuasidynamic(v, mnps, this->saved_object_trajectory[timestep].envs,
                       env_mode, this->f_gravity, this->object_inertia,
                       x_object, this->mu_env, this->mu_mnp, this->wa, this->wt,
                       h_time, this->cons.get(), QUASIDYNAMIC_RANGE);
  }
