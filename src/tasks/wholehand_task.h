...
public:
  ...
  struct State2 {
    struct Action {
      ...
      std::vector<int>
          contact_idxes; // move to which contact point on the object surface
      std::vector<std::string>
          motion_types; //"relocate", "roll", "slide", "new", "release"

      Action() {}
      Action(int timestep_, std::vector<std::string> hand_segments_,
    ...
    static bool is_no_action(Action action) { return action.timestep == -1; }

    static Action no_action() { return Action(); }
    ...
    State2(int t, std::vector<std::string> hand_segments_,
           std::vector<int> contact_idxes_)
  ...
  struct ContactConfig {
    ...
    std::vector<int>
        contact_idxes; // touches which contact point on the object surface
    ContactConfig() {}
    ContactConfig(std::vector<std::string> hand_segments_,
                  std::vector<int> contact_idxes_)
  ...
  void sample_likely_contact_configs(
      ...
      const std::vector<ContactPoint> &envs, int max_sample,
      std::vector<ContactConfig> *sampled_actions, std::vector<double> *probs);
